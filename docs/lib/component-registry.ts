...
export default function Example() {
  ...
  return (
    ...
    props: [
      ...
      { name: 'radius', type: "'full' | 'lg' | 'md' | 'sm' | 'none'", default: "'full'", description: 'Corner rounding of the track + segments (full = pill).' },
    ...
    knobs: [
      ...
      { name: 'radius', type: 'select', options: ['full', 'lg', 'md', 'sm', 'none'], default: 'full' },
  ...
  },
  {
    ...
    importName: 'Select',
    props: [
      { name: 'multiple', type: 'boolean', default: 'false', description: 'Enable multi-select. When true, value/defaultValue/onValueChange become string[]; picking an option TOGGLES it and the menu stays open. The trigger shows the single label or "N selected".' },
      { name: 'value', type: 'string | string[]', description: 'The controlled selected value — string in single mode, string[] when multiple.' },
      { name: 'defaultValue', type: 'string | string[]', description: 'The default value when uncontrolled (string, or string[] when multiple).' },
      { name: 'onValueChange', type: '(value: string | string[]) => void', description: 'Fired when the selection changes — receives a string in single mode, the full string[] when multiple.' },
      ...
      { name: 'searchable', type: 'boolean', description: 'Show a search field at the top of the dropdown that filters options by label (query resets on close; empty groups + their labels hide; shows "No results"). Omit to auto-enable for lists longer than 7 options; set explicitly to force on/off.' },
      { name: 'SelectItem icon', type: 'ReactNode', description: 'Per-option leading glyph — pass a Phosphor icon with weight="duotone" (DS convention). Sized to the option text and mirrored on the trigger when that option is selected.' },
    ],
    knobs: [
      { name: 'size', type: 'select', options: ['sm', 'md', 'lg'], default: 'md' },
...
export default function Example() {
  return (
    ...
    props: [
      ...
      { name: 'visibleKeys', type: 'string[]', description: 'Ordered visible column keys — both filters AND reorders: columns render in this array’s order, not the columns[] order. Define columns once and swap visibleKeys per variant instead of building multiple column arrays.' },
      { name: 'isRowExpanded / renderNestedRow / nestedRowClassName', type: '(row) => boolean, (row, ctx) => ReactNode, string', description: 'Expandable/nested rows: renderNestedRow is rendered as a full-colspan <tr> directly under any row where isRowExpanded returns true — for inline sub-row editors, detail panels, or nested content. nestedRowClassName styles that <td>.' },
      { name: 'rowActions / alwaysShowRowActions', type: '(row, ctx) => ReactNode, boolean', description: 'Hover-reveal action cluster pinned to the row’s trailing edge (gradient-masked overlay in the last cell), shown on row hover/focus; alwaysShowRowActions keeps it visible. Clicks don’t bubble to onRowClick.' },
      { name: 'renderBulkBar', type: '(selectedRows: T[]) => ReactNode', description: 'Sticky bulk-action bar rendered above the table whenever the selection is non-empty. Requires selection.' },
      { name: 'Column.stopRowClick', type: 'boolean (per column)', description: 'Set on a data column so clicks in that cell (a Select/dropdown, inline input) don’t bubble to onRowClick.' },
      { name: 'GroupConfig.emptyState', type: 'ReactNode (per group)', description: 'Rendered as a full-width row under a group’s band when that group has zero rows after filtering — per-group empty messaging.' },
      { name: 'ColumnFilterMenu', type: 'component', description: 'Column-header filter + sort popover (funnel icon → Sort A→Z/Z→A + a checkbox filter list), portaled with viewport flip/clamp and the shared DS Popover chrome (border + --shadow-menu). Lists longer than 7 values auto-gain a search field (list scrolls internally). Render it from a column’s headerCell. Fully controlled — bind its selected/onToggle/onClear to the same state as a TableShell toggle-group facet and the applied filter reflects in the filter band above the table.' },
...
export default function Example() {
  ...
  },
  {
    ...
    props: [
      ...
      { name: 'variant', type: "'default' | 'success' | 'warning' | 'error' | 'neutral'", default: "'default'", description: 'Visual style of the progress bar (neutral = black/charcoal).' },
      ...
      { name: 'disabled', type: 'boolean', default: 'false', description: 'Muted, non-interactive state — fill switches to the neutral grey gradient.' },
    ],
    knobs: [
      { name: 'variant', type: 'select', options: ['default', 'success', 'warning', 'error', 'neutral'], default: 'default' },
      { name: 'size', type: 'select', options: ['sm', 'md', 'lg'], default: 'md' },
...
export default function Example() {
  ...
  return (
    ...
    props: [
      ...
      { name: 'columns', type: 'readonly TableColumn[]', description: 'Provide columns to get the BUILT-IN Customize popover (per-column show/hide + drag-reorder), auto-populated from this list — pass the SAME columns your table renders (a DataTable’s Column[] is assignable directly: shared key/label/alwaysVisible). The FIRST column is FIXED (always shown, not reorderable). TableColumn = { key, label, hideable?, hidden?, alwaysVisible? }.' },
      { name: 'visibleKeys / onVisibleKeysChange', type: 'string[] / (keys) => void', description: 'STANDARD controlled model, shared with DataTable’s visibleKeys: Customize edits emit the next ordered visible keys — feed the same state to your DataTable. Visibility = membership; order = the array (hidden columns listed after). Prefer this over onColumnsChange.' },
      { name: 'onColumnsChange', type: '(cols: TableColumn[]) => void', description: 'LEGACY alternative to visibleKeys — emits the columns with mutated hidden flags + new order. Used only when visibleKeys is not set. Reflect the order + hidden flags when you render the header + cells.' },
